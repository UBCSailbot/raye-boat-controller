...
class JibeOnlyRudderController:
    ...
    def get_jibe_controller_error(current_heading, desired_heading, jibe_direction):
        ...
        if jibe_direction > 0:
            return max((2 * math.pi  - abs(error_one_way)) * -JibeOnlyRudderController.__getSign(error_one_way), error_one_way)
        else:
            return min((2 * math.pi  - abs(error_one_way)) * -JibeOnlyRudderController.__getSign(error_one_way), error_one_way)
